...
with open("mssql_seed.sql", "w", encoding="utf-8") as f:
    f.write(sql)

print("Seed file created. Executing in MSSQL container...")
time.sleep(20) # wait for MSSQL to be ready

try:
    subprocess.run(["docker", "cp", "mssql_seed.sql", "sqleditor-mssql:/mssql_seed.sql"])
    subprocess.run(["docker", "exec", "sqleditor-mssql", "/opt/mssql-tools18/bin/sqlcmd", "-S", "localhost", "-U", "sa", "-P", "Mssql_Strong_Pass123!", "-C", "-i", "/mssql_seed.sql"])
