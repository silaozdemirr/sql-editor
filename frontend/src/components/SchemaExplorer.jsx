      ...
      {isExpanded && (
        <>
          ...
          {schema && !isLoading && !error && <ul className="schema-list root-list" style={{ paddingLeft: '24px' }}>
            <SchemaGroup title="Tablolar" items={(schema.tables || []).filter(t => dbMatches || !searchTerm || t.name.toLowerCase().includes((searchTerm || "").toLowerCase()))} connectionToken={connectionInfo.connectionToken} currentDatabase={dbName} icon={FiTable} onOpenDDL={handleOpenDDL} onOpenTableData={handleOpenTableData} onCreateTable={onCreateTable ? () => onCreateTable(dbName) : undefined} onOpenMockData={onOpenMockData ? (table) => onOpenMockData(dbName, table) : undefined} />
            {schema?.views && schema.views.length > 0 && <SchemaGroup title="Görünümler" items={(schema.views || []).filter(t => dbMatches || !searchTerm || t.name.toLowerCase().includes((searchTerm || "").toLowerCase()))} connectionToken={connectionInfo.connectionToken} currentDatabase={dbName} icon={FiEye} onOpenTableData={handleOpenTableData} />}
